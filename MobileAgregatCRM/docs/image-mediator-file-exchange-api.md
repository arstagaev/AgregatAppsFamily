...
Номер документа перед отправкой нормализуется и должен содержать от 6 до 12 цифр.
...
`year/month` должны в точности совпадать со значениями из предшествующего `can-upload` для этого пакета.
...
`GET /api/v1/documents/{document_number}/images/count?document_name={type}`

Назначение: показать количество доступных фотографий. Тело запроса отсутствует. Параметры `year/month` не передаются.
...
`GET /api/v1/documents/{document_number}/images?page={page}&document_name={type}`

Назначение: получить постраничный список метаданных для просмотра. `page` начинается с 1. Тело запроса отсутствует.
...
При `503` клиент выполняет одну повторную попытку после `Retry-After` (минимум 1 секунда; если заголовка нет — через 2 секунды). Автоповтор для отправки фотографий не выполняется: повторная отправка должна использовать тот же `Idempotency-Key`.
...
| `401` | Токен недействителен; предложить повторный вход |
| `404` | Папка документа недоступна/не найдена |
| `409` | Конфликт повторной отправки по `Idempotency-Key` |
| `413` | Файл или запрос слишком большой |
| `429` | Исчерпан лимит фото документа |
| `503` | Сервис занят; для чтения контента есть одна отложенная попытка |
