...
interface IBuyerOrdersComponent : IListMaster {
    val buyerOrders: StateFlow<Resource<List<BuyerOrderDto>>>
}

class BuyerOrdersComponent(
    componentContext: ComponentContext,
    private val onBack: () -> Unit,
) : IBuyerOrdersComponent,
    ComponentContext by componentContext,
    KoinComponent {

    companion object {
        private const val PAGE_SIZE = 30
        private val BUYER_ORDERS_DEFAULT_REFINE = RefineState(
    ...
    private val _buyerOrders = MutableStateFlow<Resource<List<BuyerOrderDto>>>(Resource.Loading)
    override val buyerOrders: StateFlow<Resource<List<BuyerOrderDto>>> = _buyerOrders

    private val _refineState = MutableStateFlow(BUYER_ORDERS_DEFAULT_REFINE)
    ...
    override fun fullRefresh() {
        appScope.launch {
            _buyerOrders.value = Resource.Success(data = loadedOrders, additionalLoading = true)
            _refineState.value = loadRefineState()
            _ncount.value = 0

            val result = repository.loadBuyerOrders(0, refineState.value)
            if (result is Resource.Success) {
                ...
                _buyerOrders.value = Resource.Success(loadedOrders.toList(), additionalLoading = false)
            } else {
                _buyerOrders.value = result
            }
        }
    }
    ...
    override fun loadMore() {
        appScope.launch {
            ...
            val result = repository.loadBuyerOrders(nextOffset, refineState.value)
            if (result is Resource.Success) {
    ...
    private fun updateOrderLocally(guid: String?, transform: (BuyerOrderDto) -> BuyerOrderDto) {
        if (guid == null) return
        val index = loadedOrders.indexOfFirst { it.guid == guid }
        if (index == -1) return
        loadedOrders[index] = transform(loadedOrders[index])
        val additionalLoading = (_buyerOrders.value as? Resource.Success)?.additionalLoading ?: false
        _buyerOrders.value = Resource.Success(data = loadedOrders.toList(), additionalLoading = additionalLoading)
    }
    ...
    private fun saveRefineState(state: RefineState) {
        ...
        appSettings.setString(AppSettingsKeys.BUYER_ORDERS_REFINE_STATE, raw)
    }
}
